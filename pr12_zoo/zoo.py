...
class Animal:
    ...
    def __lt__(self, other):
        """Magic method. Defines the behaviour of the less-than operator <."""
        #  print('{!r} - Test less than {!r}'.format(self, other))
        return self.weight_range < other.weight_range

    def __gt__(self, other):
        """Magic method. Defines the behaviour of the greater-than operator >."""
        #  print('{!r} - Test less than {!r}'.format(self, other))
...
def create_animal_descriptions(animal_list: list) -> list:
    ...
    return list(map(lambda x:
                    f"{x.species} ({x.scientific_name}) lives in {x.habitat} and its diet is {x.diet}. "
                    f"These animals can live up to {x.age_up_to} years and they weigh between {x.weight_range[0]} kg "
                    f"and {x.weight_range[1]} kg as adults.", animal_list))

    # or better min(weigh_range), max(weight_range)?
