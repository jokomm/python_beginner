...
def tree(length):
    ...
    """
    if length > 5:  # https://runestone.academy/runestone/books/published/pythonds/Recursion/pythondsintro-VisualizingRecursion.html
        t.forward(length)
        t.left(60)
        tree(length * 3 / 5)
        t.right(120)
...
def apply_dragon_rules(string):
    ...
    """
    if not string:  # base case: An empty string, must return an empty string
        return ""
    ...
    if string[0] == "a":
        ...
        replacement = "LFaLb"
    return replacement + apply_dragon_rules(string[1:])
