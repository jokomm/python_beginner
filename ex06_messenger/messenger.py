...
def find_most_reacted_message(chat: Chat) -> Message:
    ...
    most_reacted = {}
    for message in chat.messages:
        most_reacted[message.reactions] = message
    return most_reacted[max(most_reacted)]
...
def count_reactions_by_chat(chats: list) -> dict:
    ...
    reactions_by_chat = {}
    for chat in chats:
        reactions_by_chat[chat.name] = count_reactions_in_chat(chat)
    return reactions_by_chat
